...
# EnclosedPorch : NA most likely means "no enclosed porch" so area is 0
replace_na(df, "EnclosedPorch", value=0)

# Fireplaces : NA most likely means 0 fireplaces (similar to FireplaceQu)
...
def assess_regression_model(model, X_train, X_test, y_train, y_test) -> tuple[float, float]:
    ...
    # your_code_here
    exp_y_pred_train = np.expm1(y_pred_train)
    exp_y_pred_test = np.expm1(y_pred_test)
    ...
    # your_code_here
    rmse_train = root_mean_squared_error(exp_y_train, exp_y_pred_train)
    rmse_test = root_mean_squared_error(exp_y_test, exp_y_pred_test)
...
# // skomentuj tutaj
#
# Na zbiorze treningowym błąd wyniósł 16751.11 dolarów, a na testowym 21318.61. Model na zbiorze treningowym otrzymał dokładniejsze wyniki (co jest normalne), ale wartości są do siebie zbliżone, więc nie uważam aby doszło do przeuczenia modelu (overfitting).
#
# Średnia wartość domu wynosi 180358.27 dolarów. W porównaniu do tej wartości otrzymane błędy wydają się stosunkowo niewielkie. Dodatkowo rozkład zmiennej SalePrice pokazuje, że 75% domów ma wartość większą lub równą 129425 dolarów. Wartość ta jest również znacznie większa od otrzymanych błędów, więc subiektywnie otrzymane błędy są małe. Uważam, że nie doszło do underfittingu. 
#
#
#
...
reg_ridge = RidgeCV(alphas=np.linspace(start=0.1, stop=100, num=1000))
reg_lasso = LassoCV(cv=5, n_alphas=1000, random_state=0)
...
# // skomentuj tutaj
#
# Jak widać wyniki otrzymane przy użyciu modelu Ridge i modelu Lasso na zbiorach testowych są dokładniejsze, niż te uzyskane przy użyciu zwykłej regresji liniowej (dla przypomnienia: RMSE TRAIN: 16751.11 RMSE TEST: 21318.61).
#
# Można zauważyć, że model Ridge otrzymał niższy błąd RMSE na zbiorze treningowym w porównaniu do Lasso - lepiej dopasowuje się do danych treningowych.
#
# Widzimy również, że różnice między wynikami na zbiorze treningowym i testowym są niewielkie, zastem nie występuje overfitting.
#
# Dodatkowo wykorzystanie CV przy metodzie lasso pozwoliło wyeliminować underfitting.
#
#
#
...
bars = ax.bar(class_counts.index.astype(str), class_counts.values, color=['red', 'lightgreen'])

ax.bar_label(bars)

ax.set_title("Class frequency bar plot")
ax.set_xlabel('Classes (0 = No, 1 = Yes)')
...
reg_logistic = LogisticRegression(penalty=None, class_weight="balanced", random_state=0)

reg_logistic.fit(X_train, y_train)

l0_y_pred = reg_logistic.predict(X_test)

l0_precision = precision_score(y_test, l0_y_pred)
l0_recall = recall_score(y_test, l0_y_pred)
l0_f1 = f1_score(y_test, l0_y_pred)

print("L0")
print(f"PRECISION: {l0_precision * 100:.2f}%")
print(f"RECALL: {l0_recall * 100:.2f}%")
print(f"F1 SCORE: {l0_f1 * 100:.2f}%")

# 2
reg_logisticCV = LogisticRegressionCV(Cs=100, cv=5, scoring="f1", class_weight="balanced", n_jobs=-1, random_state=0)

reg_logisticCV.fit(X_train, y_train)

l2_y_pred = reg_logisticCV.predict(X_test)
...
print("L2")
print(f"PRECISION: {l2_precision * 100:.2f}%")
print(f"RECALL: {l2_recall * 100:.2f}%")
print(f"F1 SCORE: {l2_f1 * 100:.2f}%")

# 3
reg_logisticCV_l1 = LogisticRegressionCV(Cs=100, cv=5, penalty="l1", solver="saga", scoring="f1", class_weight="balanced", n_jobs=-1, random_state=0)

reg_logisticCV_l1.fit(X_train, y_train)

l1_y_pred = reg_logisticCV_l1.predict(X_test)
...
print("L1")
print(f"PRECISION: {l1_precision * 100:.2f}%")
print(f"RECALL: {l1_recall * 100:.2f}%")
print(f"F1 SCORE: {l1_f1 * 100:.2f}%")
...
# %% editable=true slideshow={"slide_type": ""} tags=["ex"]
# your_code
l0_y_pred_train = reg_logistic.predict(X_train)

f1_train = f1_score(y_train, l0_y_pred_train)
f1_test = l0_f1
...
# // skomentuj tutaj
#
# 1. Czy występuje overfitting?
#
#     Wartośc F1 na zbiorze treningowym wyniosła 38.58%, a na zbiorze testowym 37.80%. Różnica między wynikami jest bardzo mała, zatem nie mamy doczynienia z overfittingiem.
#
# 3. Czy tworzenie modeli z regularyzacją ma sens w tym przypadku?
#
#     Regularyzacja ma za zadanie wprowadzać kary za złożoność modelu, aby zapobiegać overfittingowi. Różnice między wynikami z regularyzacjami (L1, L2) i bez niej są bardzo małe, co oznacza, że model nie ma tendencji do overfittingu. Z tego powodu uważam, że regularyzacja nie ma sensu w tym przypadku.
#
# 5. Co jest ważniejsze: precision czy recall?
#
#     Uważam, że ważniejsza jest precyzja, ponieważ im jej wartość jest wyższa, tym większy odsetek osób, którym wyświetlimy reklamę, będzie zainteresowany lokatą. Wysoka czułość oznaczałaby, że model dobrze identyfikowałby większość osób zainteresowanych lokatą, ale niższa precyzja oznacza, że więcej niezainteresowanych osób zobaczy reklamę, co będzie nieskuteczne, a dodatkowo kosztowne.
#
# 7. Jak moglibyśmy, nie zmieniając modelu, zmienić ich stosunek?
#
#     Można zmienić próg decyzyjny. Domyślnie wynosi on 0.5, czyli jako pozytywne zostaną zaklasyfikowane przypadki, dla których model przewiduje prawdopodobieństwo klasy pozytywnej powyżej 0.5. Jeżeli zwiększymy ten próg, spadnie ilość źle rozpoznanych pozytywów, a tym samym zwiększy się precyzja. Oczywiście, może zmienić się też wartość recall, ponieważ może spaść też ilość poprawnie rozpoznanych pozytytów.
...
categorical_pipeline = Pipeline(steps=[("one_hot_encoder", one_hot_encoder)])
numerical_pipeline = Pipeline(steps=[("poly_features", poly_features), ("standard_scaler", standard_scaler)])
...
reg_logistic_poly = LogisticRegression(penalty=None, class_weight="balanced", random_state=0, max_iter=1000)

reg_logistic_poly.fit(X_train, y_train)

y_pred_train = reg_logistic_poly.predict(X_train)
y_pred_test = reg_logistic_poly.predict(X_test)
...
# // skomentuj tutaj
#
# Wartość F1 na zbiorze treningowym jest tylko odrobinę wyższa (o 0.71 pkt. procentowego) od wartości na zbiorze testowym i nie uważam, że jest to wartość na tyle duża by podejrzewać overfitting. Zatem nie zdecydowałem się na tworzenie modeli z regularyzacją, ponieważ regularyzację stosuje się przede wszystkim, aby zapobiec przeuczeniu modelu.
